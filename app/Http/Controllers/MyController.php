...
class MyController extends BaseController
{
    public function index(Request $request,CustomRequest $requestvalid)
	{
		...
		{
			if (!$requestvalid) 
			{//проверяем валидатор если неудача вовзращаем форму регистрации
				return redirect('register')->withErrors($requestvalid);
			} 
			else 
			{ //если валидатор выполнился заносим данные из формы в сессиию
				...
				{//иначе пристваеваем картинку пустого аватара
					session(['avatar' => '']);
				...
				users (login, password, email, first_name, surname, gender, mobile, avatar, role) 
				values (?, ?, ?, ?, ?, ?, ?, ?, ?)', [
				session('login'),
				session('password'),
				session('email'),
				...
				$login = session('login');
				$id = DB::select("SELECT * from users WHERE login = '$login'");
				session(['id' => $id[0]->id]);
			}
        }
